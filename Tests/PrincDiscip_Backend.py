...
def generate_excel_report(author_id, start_year, end_year, df, total_publications, total_publications_plage, output_dir):
    print("Début de la génération du fichier Excel...")  # Message de débogage
    try:
        now = datetime.now().strftime("%Y-%m-%d_%Hh%M")
        filename = f"{now}{author_id}.xlsx"
        ...
        ws.append(["Date last updated", "29 January 2025"])
        ws.append(["Date exported", datetime.now().strftime("%d %B %Y")])
        ws.append(["Year range", f"{start_year} to {end_year}"])
        ws.append(["Total général", total_publications])
        ws.append(["Total dans la période choisie", total_publications_plage])
        ws.append([])

        # Crée une copie explicite pour éviter l'avertissement
        df = df.copy()
        ...
        df.loc[:, "Percentage Display"] = df["Percentage"].apply(lambda x: f"{x:.1f}%")

        # Format numbers to 1 decimal place
        df.loc[:, "Scholarly Output"] = df["Scholarly Output"].round(1)
        df.loc[:, "Percentage"] = df["Percentage"].round(1)

        # Sort DataFrame by Scholarly Output (highest to lowest)
        df = df.sort_values(by="Scholarly Output", ascending=False)

        # Ajouter les données triées dans la feuille Excel
        ws.append(["Subject Area", "Scholarly Output", "Percentage Display"])
        ws.cell(row=14, column=4, value="Percentage Display %")
        ...
        label_range = Reference(ws, min_col=4, min_row=start_row, max_row=end_row)
            ...
            for series in chart.series:
                if series is not None:
                    ...
                    series.dLbls.txPr = RichText(
                        p=[ParagraphProperties(defRPr=CharacterProperties(solidFill="1F4E79"))]
                    )

                    series.gapWidth = 400
                    series.graphicalProperties.line.noFill = True
                    series.graphicalProperties.solidFill = "1F4E79"
                    series.spPr.ln.noFill = True
        ...
        raise  # Relance l'erreur pour qu'elle soit capturée par le thread


output_dir = r"C:\Users\alima\Desktop\AlexStage\Code_graphique_test_project\output"
generate_excel_report(AUTHOR_ID, start_year, end_year, df, total_publications, total_publications_plage, output_dir)
